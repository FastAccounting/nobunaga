...
class TIDEExample:
    ...
    def _run(self):
        ...
        self.preds = (
            preds  # Update internally so TIDERun can update itself if :max_dets takes effect
        )
...
class TIDERun:
    ...
    def _eval_image(self, preds: list, gt: list):
        ...
        for pred_idx, pred in enumerate(preds):
            ...
            if pred["used"] is not None:
                self.ap_data.push(
                    pred["class"], pred["_id"], pred["score"], pred["used"], pred["info"]
                )
    ...
    def fix_main_per_class_errors(
        self, progressive: bool = False, error_types: list = None, qual: Qualifier = None
    ) -> dict:
        ap_data = self.ap_data
    ...
    def fix_special_errors(self, qual=None) -> dict:
        return {
            FalsePositiveError: self.fix_errors(transform=FalsePositiveError.fix).get_mAP()
            - self.ap,
            FalseNegativeError: self.fix_errors(false_neg_dict=self.false_negatives).get_mAP()
            - self.ap,
        }
...
class TIDE:
    ...
    def __init__(
        self, pos_threshold: float = 0.5, background_threshold: float = 0.1, mode: str = BOX
    ):
        ...
        for run_name, run in self.runs.items():
            ...
            if run_name in self.run_thresholds:
                ...
                if len(self.qualifiers) > 0:
                    ...
                    qAPs = [
                        f.mean([trun.qualifiers[q] for trun in thresh_runs if q in trun.qualifiers])
            ...
            # Otherwise, print just the one run we did
                ...
                if len(self.qualifiers) > 0:
                    print()
                    qAPs = [
                        run.qualifiers[q] if q in run.qualifiers else 0 for q in self.qualifiers
                    ]
                    P.print_table(
                        [
                            ["Name"] + list(self.qualifiers.keys()),
            ...
            P.print_table(
                ...
                + [
                    ...
                    for k in sorted(
                        main_per_class_errors[run_name][TIDE._error_types[0].short_name].keys()
                    )
                ],
    ...
    def get_main_errors(self):
        ...
        for run_name, run in self.runs.items():
            if run_name in self.run_main_errors:
                ...
                errors[run_name] = {
                    error.short_name: value for error, value in run.fix_main_errors().items()
                }
    ...
    def get_special_errors(self):
        ...
        for run_name, run in self.runs.items():
            if run_name in self.run_special_errors:
                ...
                errors[run_name] = {
                    error.short_name: value for error, value in run.fix_special_errors().items()
                }
    ...
    def get_confusion_matrix(self):
        ...
        for run_name, run in self.runs.items():
            ...
            P.print_table(
                ...
                + [
                    [run.gt.classes[k]] + [str(cnt) for cnt in cm[i]]
